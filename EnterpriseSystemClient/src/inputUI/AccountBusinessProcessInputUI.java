...
public class AccountBusinessProcessInputUI extends SuperInputUI {
	private MyTextField textStartTime, textendTime, textType, textCustomer,
			textoperater, textcarriage;
	...
	public AccountBusinessProcessInputUI(AccountCheckUI myUI, JFrame frame) {
		super(frame);
		accountCheckUI = myUI;
		// currentFrame=frame;
		initialize();
	...
	class ButtonListener implements ActionListener {
		AccountBusinessProcessInputUI currentPanel;

		public ButtonListener(AccountBusinessProcessInputUI panel) {
			currentPanel = panel;
		}
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				accountCheckUI.setWholeDocumentVO(wholeDocumentVO);
			...
			if (documentVOs.size() != 0) {
				accountCheckUI.showBusinessProcessSearchInfo(documentVOs);
