...
public class AccountReceiveDocumentInput extends SuperInputUI {
	...
	public AccountReceiveDocumentInput(AccountDocumentUI myPanel, JFrame frame) {
		super(frame);
	...
	private void initialize() {
		...
		textno = new MyTextField(83, 55, 160, 20);
		...
		textno.jtextfield.setText(receiptBLService.getNewID());
		...
		textcustomer.jtextfield.setEditable(false);
		...
		(choose.jbutton).addActionListener(new ButtonListener(this, BUTTON_CUSTOMER));
	...
	class ButtonListener implements ActionListener {
		...
		AccountReceiveDocumentInput currentPanel;
		int buttonID;

		public ButtonListener(AccountReceiveDocumentInput panel, int id) {
			buttonID = id;
			currentPanel = panel;
		}
	...
	private void saveDoc() {
		...
		if (!accountName.equals("")) {
			transferPrice = Double.parseDouble(textprice.jtextfield.getText());
			if (transferPrice < 0) {
				MyTipsFrame myTipsFrame = new MyTipsFrame();
				MyFailTipsPanel myFailTipsPanel = new MyFailTipsPanel(
						myTipsFrame, StaticImage.backOfSmallAmount.getImage());
				...
				if (!accountName1.equals("")) {
					transferPrice1 = Double.parseDouble(textprice1.jtextfield
							.getText());
					if (transferPrice1 < 0) {
						tag = 0;
						MyTipsFrame myTipsFrame = new MyTipsFrame();
						...
						String accountName2 = textName2.jtextfield.getText();
						// double transferPrice2=0;
						if (!accountName2.equals("")) {
							...
							if (transferPrice2 < 0) {
								...
								tag = 1;
								String remark2 = textnote2.jtextfield.getText();
								TransferLineItem transferLineItem2 = new TransferLineItem(
										accountName2, transferPrice2, remark2);
								...
								if (!accountName3.equals("")) {
									...
									if (transferPrice3 < 0) {
										...
										TransferLineItem transferLineItem3 = new TransferLineItem(
												accountName3, transferPrice3,
												remark3);
										transferList.addItem(transferLineItem3);
		...
		if (tag == 1) {
			ReceiptVO newReceiptVO = new ReceiptVO(date, no, customerID,
					customer, operaterID, operater, transferList, totalPrice,
					false, false, false);
			ResultMessage resultMessage = receiptBLService
					.addReceipt(newReceiptVO);
			if (resultMessage.equals(ResultMessage.SUCCESS)) {
				...
				mtf.getIni(mtPanel);
				accountDocumentUI.addAccountReceiveDocument(newReceiptVO);
	...
	protected void paintComponent(Graphics g) {
		...
		g.drawImage(StaticImage.backOfAccountReceiveDocumentInput.getImage(),
				0, 0, null);
	}
